...
<section class="bg-[#FAF9F5] text-[#0F0F0F] py-16">
    ...
    <div class="max-w-4xl mx-auto text-center px-4 relative">
        <input type="text" class="h-14 w-full px-4 rounded-full border-2 relative border-[#1EC27E] bg-white" />
        <div class="bg-[#1EC27E] w-15 h-15 rounded-full absolute flex justify-center items-center text-white font-bold -top-[2px] right-4">
            <i class="iconify" data-icon="fluent:search-12-filled" data-inline="false" style="font-size:34px;"></i>
        </div>
    </div>
</section>
...
<article class="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300">
  ...
  <img src="{{ $coupon->store['logo'] }}" alt="{{ $coupon->store['name'] }}" loading="lazy" class="w-14 h-14 rounded-full absolute -bottom-5 left-4 border-2 border-white shadow-md"
  />
</div>
...
<section class="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8 text-center grid grid-cols-1 md:grid-cols-5 gap-5">
    <div class="flex flex-col items-center gap-4 rounded-xl p-6 shadow-[0_0_5px_3px_rgba(0,0,0,0.07)]">
        {{-- SVG ICON --}}
   ...
   @php
    ...
    @endphp
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 px-2">
          ...
          <a href="{{region_route('store.website', ['slug' => $store->slug] ) }} " class="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300 ">
