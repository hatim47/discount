...
@section('content')
{{-- Hero Section --}}
<section class="bg-[#FAF9F5] text-[#0F0F0F] py-16">
    <div class="max-w-4xl mx-auto text-center px-4">
        <h1 class="text-5xl font-bold mb-4 capitalize">Tired of searching for discounts online?</h1>
        <p class="text-xl mb-8">Exclusive Coupons Updated Daily.</p>
    </div>
    <div class="max-w-4xl mx-auto text-center px-4 relative">
        <input type="text" class="h-14 w-full px-4 rounded-full border-2 relative border-[#1EC27E] bg-white"  @click="openSearch()" />
  <div class="bg-[#1EC27E] w-15 h-15 rounded-full absolute flex justify-center items-center text-white font-bold -top-[2px] right-4" @click="openSearch()" >
    <iconify-icon icon="flowbite:search-outline" width="34" height="34" class="text-white absolute  z-2"></iconify-icon>
</div>
    </div>
</section>
...
<article class="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300">
      <div class="relative">
  <img src="{{ $coupon['image'] }}" alt="{{ $coupon->store['name'] }}" loading="lazy" class="w-full h-40 object-cover" />
 <a href="{{region_route('store.website', ['slug' => $coupon->store['slug'] ]) }}"> <img src="{{ $coupon->store['logo'] }}" alt="{{ $coupon->store['name'] }}" loading="lazy" class="w-14 h-14 rounded-full absolute -bottom-5 left-4 border-2 border-white shadow-md"
  /></a>
</div>
...
<div class="p-4 pt-3 flex flex-col justify-between h-[170px]">
  ...
  <h2 class="text-gray-900 font-semibold text-sm">{{ $coupon->store['name'] }}</h2>
   @if($coupon->verified)
        <span class="text-[#0f0f0f] bg-[#1EC27E]/20 uppercase px-1 text-xs rounded-2xl">Verified</span>
   @endif
</div>
...
@endphp
 ...
 <button onclick="openCouponLink('{{ $coupon->store->link  }}')"
  data-code=""
    data-title="{{ $coupon->title }}"
    data-terms="{{ $coupon->terms }}" 
                    class="inline-block bg-[#1EC27E] text-white px-4 py-2 rounded hover:bg-[#1EC27E]">
                Get Deal
            </button>
              @endif
                </div>
...
<section class="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8 text-center">
    ...
    <div class="flex gap-4 flex-wrap justify-center">
        ...
        <div id="store-slider" 
     class="flex overflow-x-auto space-x-4 scrollbar-hide cursor-grab active:cursor-grabbing px-4 py-6">
    @foreach($stores as $store)
        <div class="flex-shrink-0 rounded-full border m-2">
            <img src="{{ $store['logo'] }}" 
                 alt="{{ $store['name'] }}" 
...
{{-- Stats / About Section --}}
<section class="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8 text-center grid grid-cols-1 md:grid-cols-5 gap-5">
    <div class="flex flex-col col-span-3 sm:col-span-1 items-center gap-4 rounded-xl p-6 shadow-[0_0_5px_3px_rgba(0,0,0,0.07)]">
        {{-- SVG ICON --}}
        <span class="text-5xl text-red-500 font-semibold">410</span>
        ...
        <p class="text-[#0F0F0F] text-xl font-bold">Offers added this week</p>
    </div>
    <div class="flex flex-col col-span-3 sm:col-span-1  items-center gap-4 rounded-xl p-6 shadow-[0_0_5px_3px_rgba(0,0,0,0.07)]">
        {{-- Another SVG ICON --}}
        <span class="text-5xl text-red-500 font-semibold">410</span>
    ...
    <div class="col-span-3 flex flex-col gap-4 rounded-xl p-5 shadow-[0_0_5px_3px_rgba(0,0,0,0.07)]">
        <h4 class="font-bold text-2xl text-[#0F0F0F]">About Top Vouchers Code</h4>
        <div class="overflow-auto max-h-60">
            <p>
                <b>Top Vouchers Code contains affiliate links to products. We may receive a commission for purchases made through these links.</b>
                <br>
...
<div class="bg-[#F2F0E6]">
    ...
    @foreach($categories as $category)
        <div class="group flex justify-center gap-3 items-center my-8 py-2">
            <h3 class="font-bold text-[#0F0F0F] group-hover:text-gray-500 text-3xl m-0 p-0">{{ $category->name }}</h3>
            <div class="text-[#0F0F0F] group-hover:text-gray-500 pt-2">
                <i class="iconify" data-icon="icon-park-solid:right-c" data-inline="false" style="font-size:28px;"></i>
            </div>
        </div>
   @php
    ...
    @endphp
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 px-2">
          ...
          <a href="#" class="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300 ">
