...
class Themify_Hooks {
    ...
    private CONST PRE = 'setting-hooks';
    private static $data;
    ...
    private static function check_visibility( $id ) {
        ...
        if ( ! empty( $logic ) ) {
            ...
            if (
                ...
                || ( isset($logic['general']['404']) && ( is_404() || self::is_custom_404() ) )
                || ( isset( $logic['general']['page'] ) &&  is_page() && ! is_front_page() && ! self::is_custom_404() )
                            ...
                            foreach ($logic['tax']['category_single'] as $key => $tax) {
                                ...
                                if ( $terms !== false && !is_wp_error($terms) && is_array($terms) ) {
                                    foreach ( $terms as $term ) {
                                        $term->slug = urldecode( $term->slug ); /* for non-ascii term slugs */
                ...
                if ( isset( $logic['wc'] ) && themify_is_woocommerce_active()) {
    ...
    private static function is_custom_404() {
        return class_exists( 'Themify_Custom_404' ) && Themify_Custom_404::is_custom_404();
    }
    ...
    public static function output_item() {
        ...
        foreach ( self::$action_map[ $hook ] as $id ) {
            ...
            if ( ! empty( self::$data["{$pre}-{$id}-r"] ) ) {
                ...
                self::$counter[ $id ]++;
                $max = isset( self::$data["{$pre}-{$id}-m"] ) ? (int) self::$data["{$pre}-{$id}-m"] : 0;
                if ( self::$counter[ $id ] % (int) self::$data["{$pre}-{$id}-r"] !== 0 || ( $max !== 0 && self::$counter[ $id ] / (int) self::$data["{$pre}-{$id}-r"] > $max ) ) {
            ...
            if ( ! empty( self::$data["{$pre}-{$id}-code"] ) ) {
                echo apply_filters( 'themify_hooks_item_content', self::themify_do_shortcode_wp( '<!-- hook content: ' . $hook . ' -->' . self::$data["{$pre}-{$id}-code"] . '<!-- /hook content: ' . $hook . ' -->' ), __CLASS__ );
            }
        }
    }
    ...
    public static function register_default_hook_locations() {
        foreach ( array(
            ...
            array( 'themify_mobile_menu_start', 'main_menu_wrapper_start', 'layout' ),
            array( 'themify_mobile_menu_end', 'main_menu_wrapper_end', 'layout' ),
            ...
            array( 'themify_post_before_module', 'builder_post_before', 'post_module' ),
            array( 'themify_post_start_module', 'builder_post_start', 'post_module' ),
            array( 'themify_before_post_image_module', 'builder_before_post_image', 'post_module' ),
            array( 'themify_after_post_image_module', 'builder_after_post_image', 'post_module' ),
            array( 'themify_before_post_title_module', 'builder_before_post_title', 'post_module' ),
            array( 'themify_after_post_title_module', 'builder_after_post_title', 'post_module' ),
            array( 'themify_post_end_module', 'builder_post_end', 'post_module' ),
            array( 'themify_post_after_module', 'builder_post_after', 'post_module' ),
            array( 'themify_comment_before', 'comments_before', 'comments' ),
            array( 'themify_comment_start', 'comments_start', 'comments' ),
            array( 'themify_comment_end', 'comments_end', 'comments' ),
            array( 'themify_comment_after', 'comments_after', 'comments' ),
    ...
    private static function item_template( $id, $new = false ) {
        ...
        foreach ( self::get_location_groups() as $group => $label ) {
            if ( ! empty( $locations[$group] ) ) {
                ...
                foreach ( $locations[$group] as $key => $value ) {
                    $output .= '<option class="loc-' . esc_attr( $group ) . '" value="' . $key . '" ' . selected( $current, $key, false ) . '>' . esc_html( $value ) . '</option>';
                }
                $output .= '</optgroup>';
            }
        }
        ...
        $output .= '<div class="row tf_hook_repeat"><p>
            <span data-id="post"> ' . __( 'On archive loops, output this hook content on every ', 'themify' ) . '</span>
            <span data-id="comment"> ' . __( 'Output this hook content on every ', 'themify' ) . '</span>
            <span data-id="nth-p"> ' . __( 'Show after ', 'themify' ) . '</span>
            <span data-id="nth-every-p"> ' . __( 'Show after every ', 'themify' ) . '</span>' .
            ': <input type="number" min="1" step="1" name="' . $pre . '-' . $id . '-r" value="' . themify_get( "{$pre}-{$id}-r", false, true ) . '" class="width4" />
            <span data-id="post"> ' . __( 'nth post; ', 'themify' ) . '</span>
            <span data-id="comment"> ' . __( 'nth comment;', 'themify' ) . '</span>
            <span data-id="nth-p"> ' . __( 'nth paragraph.', 'themify' ) . '</span>
            <span data-id="nth-every-p"> ' . __( 'nth paragraph.', 'themify' ) . '</span>'
            . '<span class="hook_max_repeat">' . __( 'Maximum of ', 'themify' ) . '<input type="number" min="1" step="1" name="' . $pre . '-' . $id . '-m" value="' . themify_get( "{$pre}-{$id}-m", false, true ) . '" class="width4" />' . __(' times.', 'themify' )
            . '</span></p></div>';
    ...
    private static function child_post_name($post) {
        ...
        if ( $post->post_parent > 0 ) {
            ...
            $str = self::child_post_name($parent) . '/' . $str;
        }
