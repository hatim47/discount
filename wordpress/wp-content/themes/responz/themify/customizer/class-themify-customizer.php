...
if (!class_exists('Themify_Customizer',false)) :
    ...
    class Themify_Customizer {
        ...
        function __construct() {
            ...
            add_action('wp_ajax_tf_customizer_import', array($this, 'customizer_import'));
            add_action('wp_ajax_tf_customizer_export', array($this, 'customizer_export'));

            add_action( 'init', array( $this, 'disable_menus_and_widgets' ) );

            /* load main.js */
            add_action( 'customize_controls_print_footer_scripts', [ 'Themify_Enqueue_Assets', 'js_localize' ], 1 );
        }
        ...
        function customize_control_scripts() {
            ...
            // Minicolors JS
            themify_enque_script('themify-colorpicker-js', THEMIFY_METABOX_URI . 'js/themify.minicolors.js', THEMIFY_VERSION, array('themify-main-script'),true);
            ...
            themify_enque_script('themify-combobox', THEMIFY_CUSTOMIZER_URI . '/js/jquery-scombobox.min.js', THEMIFY_VERSION, array('themify-main-script'),true);


            // Controls JS
            themify_enque_script('themify-customize-control', THEMIFY_CUSTOMIZER_URI . '/js/themify-customize-control.js', THEMIFY_VERSION, array('customize-controls', 'underscore', 'backbone', 'themify-main-script'),true);
            $controls = array(
        ...
        function save_option() {
            ...
            if ( isset( $_POST['option'], $_POST['value'] ) && in_array( $_POST['option'], [ 'blogname', 'blogdescription' ], true ) ) {
                update_option( $_POST['option'], stripslashes( trim( $_POST['value'] ) ) );
                echo 'saved';
            } else {
                echo 'notsaved';
            }
        ...
        function write_stylesheet($delete_empty = true) {
            ...
            if (!empty($css_to_save)) {
                ...
                update_option('themify_custom_fonts', !empty($this->customizer_fonts) ? $this->customizer_fonts : array(), false );
                $cf=array();
                foreach($this->customizer_cf_fonts as $k=>$v){
                    $cf[]=$k.':'.$v;
                }
                update_option('themify_custom_cf_fonts', $cf, false );
            } elseif ($delete_empty) {
                Themify_Filesystem::delete($css_file,'f');
            }
        ...
        function enqueue_custom_fonts($fonts) {
            $custom_fonts =!empty($this->customizer_cf_fonts) ? $this->customizer_cf_fonts : get_option('themify_custom_cf_fonts');
        ...
        function customizer_import() {
            ...
            check_ajax_referer( 'tf_customizer_import','nonce' );

            if ( ! empty( $_POST['data'] ) ) {
                $data = unserialize(base64_decode( $_POST['data'] ), [ 'allowed_classes' => false ] );
                if ( !empty($data) ) {
        ...
        /**
         * Export Customizer settings to file
         * @hooked to wp_ajax_tf_customizer_export
         * @package themify
         */
        function customizer_export() {
            ...
            check_ajax_referer( 'tf_customizer_export','nonce' );

            $theme = wp_get_theme();
            $mods = get_theme_mods();
            unset( $mods['sidebars_widgets'], $mods['nav_menu_locations'] );
            $mods['theme'] = strtolower( $theme->display('Name') );
            $mods['timestamp'] = dechex(time());
            wp_send_json_success( [
                'name' => $mods['theme'] . '_themify_customizer_export_' . date('Y_m_d'),
                'data' => serialize( $mods )
            ] );
        }
        ...
        function site_logo($location = '') {
            ...
            if ( $is_image_mode===true) {
				...
				if(! empty( $logo->imgwidth )){
					$attr['width']=$logo->imgwidth;
				}
				if(! empty( $logo->imgheight )){
					$attr['height']=$logo->imgheight;
				}
				unset($logo);
        ...
        function site_description($site_desc = '', $location = 'site-tagline') {
            ...
            $url = apply_filters('themify_customizer_tagline_url', !empty($desc->link) ? $desc->link : '' );
            $html = $url !== '' ? '<a class="site-taglink" href="' . esc_attr($url) .'" >' : '';
            if ( $is_image_mode===true) {
        ...
        function disable_menus_and_widgets() {
            ...
            if ( isset( $wp_customize ) && isset( $_GET['themify'] ) && $_GET['themify'] === '1' ) {
                 ...
                 */
                remove_filter( 'customize_dynamic_setting_args',          array( $wp_customize->widgets, 'filter_customize_dynamic_setting_args' ), 10 );
                ...
                remove_filter( 'is_active_sidebar',                       array( $wp_customize->widgets, 'tally_sidebars_via_is_active_sidebar_calls' ), 10 );
                remove_filter( 'dynamic_sidebar_has_widgets',             array( $wp_customize->widgets, 'tally_sidebars_via_dynamic_sidebar_calls' ), 10);
                remove_filter( 'customize_dynamic_partial_args',          array( $wp_customize->widgets, 'customize_dynamic_partial_args' ), 10);
                ...
                remove_filter( 'customize_dynamic_setting_args', array( $wp_customize->nav_menus, 'filter_dynamic_setting_args' ), 10);
                remove_filter( 'customize_dynamic_setting_class', array( $wp_customize->nav_menus, 'filter_dynamic_setting_class' ), 10 );
                ...
                remove_filter( 'customize_dynamic_partial_args', array( $wp_customize->nav_menus, 'customize_dynamic_partial_args' ), 10);
            }
        }
    }
